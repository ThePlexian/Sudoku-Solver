namespace SudokuSolver.Model.Data
{
    public partial class Sudoku
    {
        ...
        {
            bool success = st switch {
                SolvingTechnique.HumanSolvingTechnique => SolveHumanLike(),
                SolvingTechnique.BackTracking => SolveBackTracking(),
                _ => false,
            };
            SolvingCompleted?.Invoke(this, EventArgs.Empty);
            ...
            foreach (var box in boxes) {
                ...
                foreach (int n in selectedcandidates) {
                    ...
                    if (IsIdenticalRow(selectedcells, out int rowindex)) {
                        var row = GetCellsInRow(new GridIndex(rowindex, 0));
                        ...
                        var column = GetCellsInColumn(new GridIndex(0, columnindex));
        ...
        private List<Cell> GetCellsInRow(GridIndex ind)
        {
        ...
        private List<Cell> GetCellsInColumn(GridIndex ind)
        {
            var temp = new List<Cell>();
        ...
        private List<Cell> GetCellsInBox(GridIndex ind)
        {
            var temp = new List<Cell>();
        ...
        private List<Cell> GetConnectedCells(GridIndex ind) => GetCellsInRow(ind).Concat(GetCellsInColumn(ind)).Concat(GetCellsInBox(ind)).Where(c => !c.IsEqual(Cells[ind.Row, ind.Column])).ToList();
        ...
        private IEnumerable<List<Cell>> GetAllClusters() => GetListOfRows().Concat(GetListOfColumns()).Concat(GetListOfBoxes()).ToList();
        ...
        private static bool IsIdenticalBox(IList<Cell> cells, out GridIndex boxindex)
        {
        ...
        private static GridIndex ToBoxIndex(GridIndex i) => new(i.Row / 3 * 3, i.Column / 3 * 3);
        ...
        private bool RemoveCandidates(IEnumerable<Cell> cells, params int[] candidates) => cells.Aggregate(false, (current, c) => current | RemoveCandidates(c, candidates));


        //Deletes the number out of the candidates of the reachable Cells
        private void DeleteCandidate(int candidate, GridIndex i)
        {
        ...
        private void AddCandidate(int candidate, GridIndex i)
        {
            if (candidate == 0) {
                return;
            }
        ...
        private void RefreshCandidates(GridIndex i)
        {
